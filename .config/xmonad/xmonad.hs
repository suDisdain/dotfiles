...
myLayout =
  spacingRaw True (Border 0 5 5 5) True (Border 7 7 7 7) True
    $   ResizableTall 1 (3 / 100) (58 / 100) []
    ||| spiral (107 / 125)
    ||| simpleFloat
myFont :: [Char]
...
myManageHook =
  (isFullscreen --> doFullFloat) <+> insertPosition End Newer <+> composeAll
    [ className =? "Oxygen" --> doCenterFloat
    , className =? "Electron" --> doCenterFloat
    ]
